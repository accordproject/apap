...
**Docker build context is the repo root, not `server/`.** `server/compose.yaml` sets `context: ..` with `dockerfile: server/Dockerfile` so the image can bundle `model/protocol.cto` alongside the compiled server (the MCP `apap://schema/protocol.cto` resource reads it at runtime). A bare `docker build .` from inside `server/` will fail on missing `model/`; use `docker compose up`, or `docker build -f server/Dockerfile ..` from the parent.
...
- **Typed errors live in `services/errors.ts`; the rollout is partial.** The `ServiceError` hierarchy (`TemplateNotFoundError`, `AgreementNotFoundError`, `AgreementConversionError`, `InvalidPayloadError`, `ValidationError`) carries a code, HTTP status, and structured details, and serializes via `toJSON()`. `handlers/agreements.ts` is the only handler currently wired — its catch blocks do `if (error instanceof ServiceError) { res.status(error.statusCode).json(error.toJSON()) }`. Other handlers still throw bare `Error('...')` strings; when you touch one, migrate it to the typed errors rather than adding more bare throws. Anything that is not a `ServiceError` is a real 500.
...
`loadProtocolCto()` probes two candidate paths (`__dirname/../../../model/protocol.cto` for `dist/handlers`, `__dirname/../../model/protocol.cto` for source/jest) and caches the first hit. If you restructure the dist layout or move the model, update the candidates rather than adding a build-time copy step.
