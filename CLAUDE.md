...
- **MCP calls the service layer directly.** `handlers/mcp.ts` takes a `Database` handle via `getServer(db)` and passes it into `services/`. There is no internal HTTP loop: `makeApiRequest`, `API_BASE_URL` and `APAP_API_AUTH_HEADER` are gone, and REST and MCP share one implementation per operation (both creation paths call `createAgreement`). When porting from `apap-mcp-poc/`, expect the call site to be a service call, not a fetch. A new MCP tool that reaches for `fetch` against this server's own REST API is reintroducing the retired pattern.
- **Template-fetching uses a scheme-dispatch retriever pattern.** `handlers/retrievers/ITemplateRetriever.ts` declares `getURISchemes()` + `fetch()`. `services/agreementService.ts:createAgreement` iterates the retriever list — injectable via `options.templateRetrievers`, defaulting to `[new HttpTemplateRetriever()]` — and picks the first whose scheme matches the agreement's `template` URI. The fetched `.cta` is hashed and cached by `Template.hash` (unique). To add a new scheme (e.g. `s3://`, `ipfs://`), implement `ITemplateRetriever` and push to `availableRetrievers`.
...
MCP handlers hold a `Database` handle directly, so there is no `API_BASE_URL` to configure for them and a reverse proxy in front of the RI does not affect the MCP path.
